...
class CodeObject(Nameable):
    ...
    def is_available(cls):
        ...
        raise NotImplementedError(
            "CodeObject class %s is missing an "
...
def _error_msg(code, name):
    ...
    if len(code_lines) <= 1:
        ...
        error_msg += ("from %d lines of abstract code, first line is: " '"%s"\n') % (
            len(code_lines),
            code_lines[0],
        )
    return error_msg
...
def check_compiler_kwds(compiler_kwds, accepted_kwds, target):
    ...
    for key, value in compiler_kwds.items():
        if key not in accepted_kwds:
            formatted_kwds = ", ".join("'{}'".format(kw) for kw in accepted_kwds)
            raise ValueError(
                "The keyword argument '{}' is not understood by "
                "the code generation target '{}'. The valid "
                "arguments are: {}.".format(key, target, formatted_kwds)
            )
...
def create_runner_codeobj(
    ...
    if template is None:
        ...
        raise AttributeError(
            ('"%s" does not provide a code generation ' 'template "%s"')
            % (codeobj_class_name, template_name)
        )
        ...
        if cond_write_var is not None:
            if (
                ...
                logger.diagnostic(
                    ...
                    % (cond_write_var.name, var.name, variables[cond_write_var.name])
                )
            else:
        ...
        if isinstance(value, Function):
            try:
                ...
                if codeobj_class is NumpyCodeObject:
                    ...
                    raise NotImplementedError(
                        ("Cannot use function " "%s: %s") % (varname, ex)
                    ) from ex
