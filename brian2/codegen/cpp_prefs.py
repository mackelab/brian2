...
if platform.system() == "Windows":
    ...
    if os.path.isfile(flag_file):
        try:
            ...
            logger.debug(
                'Opening file "{}" to get CPU flags failed with error '
    ...
    if flags is None:  # If we don't have stored info, run get_cpu_flags.py
        ...
        try:
            output = subprocess.check_output(
                [sys.executable, get_cpu_flags_script], universal_newlines=True
            )
            ...
            try:
                ...
                logger.debug(
                    'Writing file "{}" to store CPU flags failed with '
                    'error "{}".'.format(flag_file, str(ex))
                )
        except subprocess.CalledProcessError as ex:
            logger.debug(
                "Could not determine optimized MSVC flags, "
    ...
    def __call__(self, *args, **kwargs):
        if not compiler_supports_c99():
            raise NotImplementedError(
                'The "{}" function needs C99 compiler ' "support".format(self.name)
            )
