...
def valid_gsl_dir(val):
    ...
    if not isinstance(val, str):
        raise PreferenceError(
            ("Illegal value for GSL directory: %s, " "has to be str" % (str(val)))
        )
    if not os.path.isdir(val):
        raise PreferenceError(
            (
                "Illegal value for GSL directory: %s, "
...
class GSLCodeGenerator(object):
    ...
    def diff_var_to_replace(self, diff_vars):
        ...
        for var, diff_num in list(diff_vars.items()):
            ...
            replace_what = "{var} = {array_name}[{idx_name}]".format(
                array_name=array_name, idx_name=idx_name, var=var
            )
    ...
    def yvector_code(self, diff_vars):
        ...
        for var, diff_num in list(diff_vars.items()):
            ...
            fill_y += [
                (
                    "\t_GSL_y[%d] = _GSL_dataholder{access_pointer}%s["
                    "_idx]{end_statement}" % (diff_num, array_name)
                )
            ]
            empty_y += [
                (
                    "\t_GSL_dataholder{access_pointer}%s[_idx] = _GSL_y["
            ...
            for var, error in list(abs_per_var.items()):
                ...
                if not var in diff_vars:
                    if not var in self.variables:
                        raise KeyError(
                            "absolute_error specified for variable "
    ...
    def to_replace_vector_vars(self, variables_in_vector, ignore=frozenset()):
        ...
        for var, var_obj in list(variables_in_vector.items()):
            ...
            if isinstance(var_obj, ArrayVariable):
                ...
                to_replace[pointer_name] = (
                    "_GSL_dataholder" + access_pointer + pointer_name
                )
            else:
        ...
        if t_in_code is not None:
            ...
            replace_what = "{t_declare} = {array_name}[0]" "{end_statement}".format(
                t_declare=t_declare, array_name=array_name, end_statement=end_statement
            )
    ...
    def translate_scalar_code(
        ...
        for line in code_lines:
            ...
            if var in list(variables_in_scalar.keys()):
                ...
                code += [
                    "_GSL_dataholder.{var} {op} {expr} {comment}".format(
    ...
    def add_meta_variables(self, options):
        if options["use_last_timestep"]:
            ...
            pointer_last_timestep = (
                self.get_array_name(self.variables["_last_timestep"]) + "[_idx]"
            )
        else:
        ...
        if options["save_failed_steps"]:
            ...
            pointer_failed_steps = (
                self.get_array_name(self.variables["_failed_steps"]) + "[_idx]"
            )
        else:
        ...
        if options["save_step_count"]:
            ...
            pointer_step_count = (
                self.get_array_name(self.variables["_step_count"]) + "[_idx]"
            )
        else:
    ...
    def translate(
        ...
        if any([var in self.variables for var in reserved_variables]):
            ...
            raise ValueError(
                (
                    "The variables %s are reserved for the GSL "
        ...
        for vs in vector_statements.values():
            ...
            try:
                ...
                if len(vs) <= 1:
                    ...
                    error_msg = (
                        "%_GSL_driver lines of abstract code, first line is: " '"%s"\n'
                    ) % (len(vs), vs[0])
...
class GSLCythonCodeGenerator(GSLCodeGenerator):
    ...
    def var_replace_diff_var_lhs(self, var, ind):
        return {
            "_gsl_{var}_f{ind}".format(var=var, ind=ind): "f[{ind}]".format(ind=ind)
        }
    ...
    def unpack_namespace_single(self, var_obj, in_vector, in_scalar):
        ...
        if isinstance(var_obj, ArrayVariable):
            ...
            if in_vector:
                code += [
                    '_GSL_dataholder.{var} = _namespace["{var}"]'.format(
                        var=var_obj.name
                    )
                ]
            if in_scalar:
...
class GSLCPPCodeGenerator(GSLCodeGenerator):
    ...
    def var_replace_diff_var_lhs(self, var, ind):
        ...
        try:
            if "unless refractory" in self.variable_flags[var]:
                return {
                    "_gsl_{var}_f{ind}".format(var=var, ind=ind): f,
                    "{scalar_dtype} _gsl_{var}_f{ind};".format(
                        var=var, ind=ind, scalar_dtype=scalar_dtype
                    ): "",
                    "{scalar_dtype} {f};".format(f=f, scalar_dtype=scalar_dtype): "",
                }  # in case the replacement
                # of _gsl_var_find to f[ind] happens first
        except KeyError:
            pass
        return {
            "const {scalar_dtype} _gsl_{var}_f{ind}".format(
                scalar_dtype=scalar_dtype, var=var, ind=ind
            ): f
        }
    ...
    def unpack_namespace_single(self, var_obj, in_vector, in_scalar):
        if isinstance(var_obj, ArrayVariable):
            ...
            if in_vector:
                return "_GSL_dataholder.{ptr} = {array};".format(
                    ptr=pointer_name, array=array_name
                )
            else:
                return ""
        else:
