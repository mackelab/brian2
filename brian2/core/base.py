...
class BrianObject(Nameable):
    ...
    def __repr__(self):
        description = (
            "{classname}(clock={clock}, when={when}, order={order}, name={name})"
        )
...
class BrianObjectException(Exception):
    ...
    def __init__(self, message, brianobj):
        ...
        logger.diagnostic(
            'Error was encountered with object "{objname}":\n{fullstack}'.format(
                objname=self._brian_objname, fullstack=brianobj._full_creation_stack
            )
        )
