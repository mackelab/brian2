...
def check_dimensions(expression, dimensions, variables):
    ...
    err_msg = (
        "Expression {expr} does not have the " "expected unit {expected}"
    ).format(expr=expression.strip(), expected=repr(get_unit(dimensions)))
...
def check_units_statements(code, variables):
    ...
    for line in code:
        ...
        if op in ("+=", "-=", "*=", "/=", "%="):
            ...
            expr = "{var} {op_first} {expr}".format(
                var=varname, op_first=op[0], expr=expr
            )
        elif op == "=":
            pass
        else:
        ...
        if varname in variables:
            ...
            raise AssertionError(
                (
                    'Variable "%s" is neither in the variables '
