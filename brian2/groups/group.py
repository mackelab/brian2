...
def _conflict_warning(message, resolutions):
    ...
    if len(resolutions) == 0:
        ...
        second_part = (
            "but the name also refers to a variable in the %s "
            "namespace with value %s."
        ) % (resolutions[0][0], _display_value(resolutions[0][1]))
    else:
        second_part = (
            "but the name also refers to a variable in the following " "namespaces: %s."
        ) % (", ".join([r[0] for r in resolutions]))

    logger.warn(
        message + " " + second_part, "Group.resolve.resolution_conflict", once=True
    )
...
def get_dtype(equation, dtype=None):
    ...
    if isinstance(dtype, Mapping):
        if equation.varname in dtype:
            ...
            if not provided_dtype.kind in BASIC_TYPES[equation.var_type]:
                raise TypeError(
                    (
                        "Error determining dtype for variable %s: %s "
                        "is not a correct type for %s variables"
                    )
                    % (equation.varname, provided_dtype.name, equation.var_type)
                )
            else:
    ...
    if equation.var_type == BOOLEAN:
        ...
        if dtype is not None:
            ...
            if not dtype.kind == "f":
                raise TypeError(("%s is not a valid floating point " "dtype") % dtype)
            return dtype
        else:
            return prefs["core.default_float_dtype"]
    else:
        raise ValueError(
            ("Do not know how to determine a dtype for " "variable %s of type %s")
            % (equation.varname, equation.var_type)
        )
...
class Indexing(object):
    ...
    def __call__(self, item=slice(None), index_var=None):
        ...
        if isinstance(item, tuple):
            raise IndexError(
                ("Can only interpret 1-d indices, " "got %d dimensions.") % len(item)
            )
        else:
...
class VariableOwner(Nameable):
        ...
        if not hasattr(self, "_group_attribute_access_active") or name in self.__dict__:
            ...
            if len(suggestions) == 1:
                ...
                error_msg += " Did you mean to write any of the following: %s ?" % (
                    ", ".join(
                        ['"%s%s"' % (suggestion, suffix) for suggestion in suggestions]
                    )
                )
            error_msg += (
    ...
    def add_attribute(self, name):
        ...
        if name in self.variables:
            raise AttributeError(
                'Cannot add an attribute "%s", it is already '
...
class Group(VariableOwner, BrianObject):
    ...
    def _resolve_external(
        ...
        if internal_variable is not None and user_identifier:
            ...
            if len(filtered_matches) == 0:
                ...
                warning_message = (
                    '"{name}" is an internal variable of group '
                    '"{group}", but also exists in the '
                )
                if len(matches) == 1:
                    warning_message += (
                        "{namespace} namespace with the value " "{value}. "
                    ).format(
                    ...
                    warning_message += (
                        "following namespaces: " "{namespaces}. "
                    ).format(
                        namespaces=" ,".join(match[0] for match in filtered_matches)
                    )
                warning_message += "The internal variable will be used."
                logger.warn(
                    warning_message.format(name=identifier, group=self.name),
        ...
        if not isinstance(resolved, (Function, Variable)):
            ...
            if value.shape != ():
                raise KeyError(
                    "Variable %s was found in the namespace, but is"
    ...
    def runner(self, *args, **kwds):
        raise AttributeError(
            "The 'runner' method has been renamed to " "'run_regularly'."
        )
