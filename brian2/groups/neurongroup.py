...
def check_identifier_pre_post(identifier):
    ...
    if identifier.endswith("_pre") or identifier.endswith("_post"):
        raise ValueError(
            '"%s" cannot be used as a variable name, the "_pre" '
...
def to_start_stop(item, N):
    ...
    if isinstance(item, slice):
        ...
        if not np.issubdtype(np.asarray(item).dtype, np.integer):
            raise TypeError(
                "Subgroups can only be constructed using integer " "values."
            )
        start = int(item[0])
        stop = int(item[-1]) + 1
        step = 1
    ...
    def _get_refractory_code(self, run_namespace):
        ...
        if ref is False:
            ...
            if prefs.legacy.refractory_timing:
                ...
                abstract_code = (
                    "not_refractory = timestep(t - lastspike, dt) >= timestep(%f, dt)\n"
                    % ref
                )
        else:
            ...
            if dims is second.dim:
                if prefs.legacy.refractory_timing:
                    ...
                    abstract_code = (
                        "not_refractory = timestep(t - lastspike, dt) >= timestep(%s, dt)\n"
                        % ref
                    )
...
class SubexpressionUpdater(CodeRunner):
    ...
    def __init__(self, group, subexpressions, when="before_start"):
        ...
        for subexpr in subexpressions.ordered:
            code_lines.append("{} = {}".format(subexpr.varname, subexpr.expr))
        code = "\n".join(code_lines)
        CodeRunner.__init__(
            self,
            group,
...
class Thresholder(CodeRunner):
    ...
    def update_abstract_code(self, run_namespace):
        ...
        if not is_boolean_expression(code, variables):
            raise TypeError(
                ('Threshold condition "%s" is not a boolean ' "expression") % code
            )
...
class NeuronGroup(Group, SpikeSource):
    ...
    def __init__(
        ...
        Group.__init__(self, dt=dt, clock=clock, when="start", order=order, name=name)
        if dtype is None:
            dtype = {}
        ...
        if not isinstance(model, Equations):
            raise TypeError(
                ("model has to be a string or an Equations " 'object, is "%s" instead.')
                % type(model)
            )
        ...
        logger.diagnostic(
            "Creating NeuronGroup of size {self._N}, "
        ...
        if threshold is not None and (reset is None and refractory is False):
            if not ("rand(" in threshold or "randn(" in threshold):
                logger.warn(
                    "The NeuronGroup '{}' sets a threshold but "
        ...
        for event_name in events.keys():
            ...
            if not _valid_event_name(event_name):
                raise TypeError(
                    ("The name '%s' cannot be used as an event " "name.") % event_name
                )
    ...
    def state(self, name, use_units=True, level=0):
        try:
            ...
            if name in self._linked_variables:
                raise TypeError(
                    ("Link target for variable %s has not been " "set.") % name
                )
            else:
                raise ex
        ...
        if not hasattr(self, "_group_attribute_access_active") or key in self.__dict__:
            ...
            if eq.dim is not linked_var.dim:
                raise DimensionMismatchError(
                    ("Unit of variable %s does not " "match its link target %s")
                    % (key, linked_var.name)
                )
            ...
            if value.index is not None:
                try:
                    ...
                    raise TypeError(
                        (
                            "The index for a linked variable has "
                ...
                if not index_array.ndim == 1 or size != len(self):
                    raise TypeError(
                        ...
                        % (key, len(self), str(index_array.shape))
                    )
            ...
            log_msg = (
                "Setting {target}.{targetvar} as a link to " "{source}.{sourcevar}"
            ).format(
                target=self.name,
                targetvar=key,
