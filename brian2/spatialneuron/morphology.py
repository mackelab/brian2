    ...
    def __getitem__(self, item):
        if isinstance(item, str):
            raise NotImplementedError(
                ("Morphologies do not support string " "indexing")
            )
...
def _add_coordinates(
    ...
    if not overwrite_existing and orig_morphology.x is not None:
        ...
        if isinstance(orig_morphology, Cylinder) and compartment_randomness == 0:
            ...
            raise NotImplementedError(
                (
                    "Do not know how to deal with section of "
        ...
        if name in self._named_children and self._named_children[name] is not subtree:
            if automatic_name:
                ...
                raise AttributeError(
                    "The name %s is already used for a " "subtree." % name
                )
...
class Morphology(object, metaclass=abc.ABCMeta):
    ...
    def __init__(self, n, type=None):
        ...
        if self._n != n:
            raise TypeError(
                "The number of compartments n has to be an integer " "value."
            )
        if n <= 0:
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, slice):  # neuron[10*um:20*um] or neuron[1:3]
            ...
            if float(item) < 0 or float(item) > (1 + 1e-12) * l[-1]:
                raise IndexError(
                    (
                        "Invalid index %s, has to be in the interval "
            ...
            if item >= self.n:
                raise IndexError(
                    ("Invalid index %d " "for %d compartments") % (item, self.n)
                )
            i = item
            j = i + 1
    ...
    def _replace_three_point_soma(compartment, all_compartments):
        ...
        if (
            ...
            if not all(abs(c.diameter - soma_c[0].diameter) < 1e-15 for c in soma_c):
                ...
                raise ValueError(
                    'Found a "3-point-soma" (lines: %s), but not '
    ...
    def from_points(points, spherical_soma=True):
        ...
        for counter, point in enumerate(points):
            if len(point) != 7:
                raise ValueError(
                    "Each point needs to be described by 7 "
            ...
            if parent_idx == index:
                raise ValueError(
                    "Compartment %d lists itself as the parent " "compartment." % index
                )
            ...
            if counter == 0 and parent_idx == -1:
                ...
                raise ValueError(
                    (
                        "Did not find the compartment %d (parent "
            ...
            for line_no, line in enumerate(f):
                ...
                if len(splitted) != 7:
                    raise ValueError(
                        ...
                        "contains %d." % (line_no + 1, len(splitted))
                    )
...
class Soma(Morphology):
    ...
    def __repr__(self):
        s = "{klass}(diameter={diam!r}".format(
            klass=self.__class__.__name__, diam=self.diameter[0]
        )
        if self._x is not None:
            s += ", x={x!r}, y={y!r}, z={z!r}".format(
...
class Section(Morphology):
    ...
    def __init__(
        ...
        if diameter.ndim != 1 or len(diameter) != n + 1:
            raise TypeError(
                "The diameter argument has to be a one-dimensional "
        ...
        if (x is not None or y is not None or z is not None) and length is not None:
            raise TypeError(
                "Cannot specify coordinates and length at the same " "time."
            )
        ...
        if length is not None:
            ...
            if length.ndim != 1 or len(length) != n:
                raise TypeError(
                    "The length argument has to be a "
            ...
            for name, value in [("x", x), ("y", y), ("z", z)]:
                if value is not None and (value.ndim != 1 or len(value) != n + 1):
                    raise TypeError(
                        ("%s needs to be a 1-dimensional array " "of length %d.")
                        % (name, n + 1)
                    )
            self._x = (
    ...
    def __repr__(self):
        ...
        s = "{klass}(diameter={diam!r}".format(klass=self.__class__.__name__, diam=diam)
        if self.n != 1:
            s += ", n={n}".format(n=self.n)
        if self._x is not None:
            s += ", x={x!r}, y={y!r}, z={z!r}".format(x=self._x, y=self._y, z=self._z)
        else:
        ...
        if length is not None:
            ...
            for name, value in [("x", x), ("y", y), ("z", z)]:
                if value is not None and (value.ndim != 1 or len(value) != 2):
                    raise TypeError(
                        "%s needs to be a 1-dimensional array of "
