...
def extract_method_options(method_options, default_options):
    ...
    for key in method_options:
        if key not in default_options:
            ...
            raise KeyError(
                'method_options specifies "{key}", but this '
...
class StateUpdateMethod(object, metaclass=ABCMeta):
    ...
    def register(name, stateupdater):
        ...
        if name in StateUpdateMethod.stateupdaters:
            raise ValueError(
                ('A stateupdater with the name "%s" ' "has already been registered")
                % name
            )
        ...
        if not isinstance(stateupdater, StateUpdateMethod):
            raise ValueError(
                (
                    "Given stateupdater of type %s does not seem to "
    ...
    def apply_stateupdater(
        ...
        if isinstance(method, Iterable) and not isinstance(method, str):
            ...
            if method[0] == the_method:
                ...
                timing = ("took %.2fs, trying other methods took " "%.2fs") % (
                    one_method_time,
            ...
            logger.info(
                msg_text.format(
                    group_name=group_name, method=the_method, timing=timing
                ),
                "method_choice",
            )
        else:
            if hasattr(method, "__call__"):
                ...
                if stateupdater is None:
                    raise ValueError(
                        'No state updater with the name "%s" ' "is known" % method
                    )
            else:
                raise TypeError(
                    (
            ...
            if group_name is not None:
                logger.debug(
                    (
                        "Group {group_name}: using numerical integration "
                        "method {method} ({timing})"
                    ).format(group_name=group_name, method=method, timing=timing),
                    "method_choice",
                )
            else:
                logger.debug(
                    (
                        "Using numerical integration method: {method} " "({timing})"
                    ).format(method=method, timing=timing),
                    "method_choice",
                )
